    ...
    public class DatabaseContext : IDatabaseContext
    {
        ...
        {            
            ...
            else
            {
                ...
                {
                    DatebaseDateTimeFixer.Run(this.trackingManager, this.connection);

                    this.folders = new List<IFolder>(this.connection.Table<Folder>());
                    this.trackingManager.Event(TrackingSource.Storage, "FormatException", "Fixed");
                }
                ...
                foreach (var task in this.tasks.OfType<Task>().Where(t => t.ParentId == null))
                {
                    if (task.ContextId >= 0)
        ...
        {
            return new SQLiteConnection(this.sqLitePlatform, path, SQLiteOpenFlags.ReadWrite | SQLiteOpenFlags.Create, false);
        }
        ...
        private class ObjectTracker
        {
            ...
            private readonly bool automaticSave;
            private Timer timer;
                ...
                lock (this.unsavedEditedItems)
                {
                    if (!this.unsavedEditedItems.Contains(sender))
                    {
                        ...
                        if (this.automaticSave)
                        {
                            if (this.timer != null)
                            {
                                this.timer.Change(TimeSpan.FromSeconds(IntervalForAutomaticSaveSecond), TimeSpan.FromMilliseconds(-1));
                            }
                            else
                            {
                                this.timer = new Timer(this.OnTimerTick, null, TimeSpan.FromSeconds(IntervalForAutomaticSaveSecond), TimeSpan.FromMilliseconds(-1));
                            }
                        }
                    }
                }
